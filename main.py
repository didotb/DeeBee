...
try:
    ...
    print(
        "weather.py is missing. Bot will not be able to use weather command.")
    pass
try:
    from keep_alive import keep_alive
except:
    print(
        "keep_alive.py is missing. Although optional, UptimeRobot will not be able to monitor bot uptime status."
    )
    pass
...
negative = ['false', 'f']
msgTrigA = [
    'wiggle', 'jiggle', 'popcat', 'catjam', 'new pc', 'blobdance', 'pepeds',
    'cum'
]
msgTrig = ['bij', 'bitch']
msgYEP = [
    'sock', 'cock', 'rock', 'dock', 'duck', 'stock', 'clock', 'croc', 'lock',
    'knock', 'mock', 'jock'
]
bot = commands.Bot(command_prefix=listener,
                   intents=discord.Intents.all(),
                   owner_id=int(os.environ['discord_user_db']),
...
async def act_app(vcid, app_name=None):
    try:
        vcal = await bot.vca.create_link(vcid,
                                         valid_app.get(app_name, None),
                                         max_age=300)
        if vcal is not None:
...
async def debug(cmsg: str = None,
                pmsg: int = None,
                cid: int = int(os.environ['discord_channel_debug'])):
    if pmsg is not None:
        switcher = {
            ...
            5:
            "Info:5: Restart initiated but failed due to invalid variable value",
            6: "Info:6: Restart initiated"
        }
        message = switcher.get(
            pmsg, "Warning: Error getting pre-determined message")
    ...
    ts = datetime.datetime.now(datetime.timezone.utc).astimezone(
        pytz.timezone('Asia/Manila')).strftime('(%b %d, %Y - %X): ')
...
if 'weather' in sys.modules:
    ...
    @tasks.loop(
        hours=1
    )  #time=[datetime.time(h,0,0,0,tzinfo=datetime.timezone(datetime.timedelta(hours=8))) for h in [0,4,8,10,12,14,16,20]]
    async def sched_weather():
        ...
        channel = bot.get_channel(
            int(os.environ['discord_channel_vii_weather']))
        ...
        if hours in [0, 4, 8, 10, 12, 14, 16, 20]:
            ...
            async with channel.typing():
                ...
                await channel.send(f"Set interval daily weather images",
                                   file=discord.File('out.mp4'))
...
async def xmas_loop():
    dt = datetime.datetime.now(datetime.timezone.utc).astimezone(
        pytz.timezone('Asia/Manila'))
    sept = datetime.datetime(dt.year, 9,
                             1).astimezone(pytz.timezone('Asia/Manila'))
    xmas = datetime.datetime(dt.year, 12,
                             25).astimezone(pytz.timezone('Asia/Manila'))
    start = xmas - sept
    delta = xmas - dt
    hours = int(dt.strftime('%H'))
    await debug(cmsg=f"(xmas): Another hour has passed: {hours}")
    channel = bot.get_channel(
        int(os.environ['discord_channel_vii_days_before_xmas']))
    ...
    if trivia['response_code'] != 0:
        await debug(
            cmsg=
            f"Error in xmas_loop: response_code returned {trivia['response_code']}. Expected 0"
        )
        return
...
async def on_ready():
    ...
    assets = {
        'large_image': '698193248190988388',
        'small_image': '698193246429380729',
        'large_text': 'Large icon test',
        'small_text': 'Small icon test'
    }
    buttons = [{
        'label': 'Rick Roll',
        'url': 'https://www.youtube.com/watch?v=dQw4w9WgXcQ'
    }]
    activity = discord.Activity(application_id=app_id,
                                assets=assets,
                                name=actName,
                                type=act,
                                state='Testing')
...
async def on_message(msg):
    ...
    if (msg.author == bot.user) and (
        (msg.channel.id == int(os.environ['discord_channel_vii_weather'])
         and msg.attachments)) or (msg.channel.id == int(
             os.environ['discord_channel_vii_days_before_xmas'])):
        ...
        for cont in mTrigA:
            if cont in {'wiggle', 'jiggle'}:
                if msg.reference is None:
                    ...
                    refmsg = await msg.channel.fetch_message(
            ...
            elif ('cum' in cont) and (msg.channel.guild.id not in {
                    os.environ['discord_guild_beaneyboo_server'],
                    os.environ['discord_guild_foxy_server'],
                    os.environ['discord_guild_falcon_server'],
                    os.environ['discord_guild_anjellying_server']
            }):
    ...
    if (bot.user in msg.mentions) or ("bitches" in msg.content.lower()):
        await msg.channel.send(content=await bot_startled(str(msg.author.id)),
                               reference=msg,
                               mention_author=True)

    if 'new pc' in msg.content.lower():
        thisEmote = await emote(False, "mmmseks")
        await msg.channel.send(content=thisEmote + thisEmote + thisEmote,
                               reference=msg,
                               mention_author=True)
    ...
    if msg.guild.id != int(os.environ['discord_guild_REDACTED_1_server']
                           ) and msg.guild.id != int(
                               os.environ['discord_guild_beaneyboo_server']
                           ) and msg.guild.id != int(
                               os.environ['discord_guild_debug_server']):
        if msg.attachments:
            channel = bot.get_channel(
                int(os.environ['discord_channel_images_archive']))
            for i in msg.attachments:
                await channel.send(details, file=await i.to_file())
        if not msg.attachments:
            channel = bot.get_channel(
                int(os.environ['discord_channel_text_archive']))
            await channel.send(details)

    if msg.guild.id == int(os.environ['discord_guild_foxy_server']):
        channel = bot.get_channel(
            int(os.environ['discord_channel_foxy_archive']))
        if msg.attachments:
            for i in msg.attachments:
                att = await i.to_file()
    ...
    if msg.guild.id == int(os.environ['discord_guild_beaneyboo_server']) and (
            msg.channel.id != int(
                os.environ['discord_channel_beaneyboo_mods_priv'])
            and msg.channel.id != int(
                os.environ['discord_channel_beaneyboo_moderator_only'])
            and msg.channel.id != int(
                os.environ['discord_channel_beaneyboo_msg_archiving'])):
        channel = bot.get_channel(
            int(os.environ['discord_channel_beaneyboo_msg_archiving']))
        if msg.attachments:
            for i in msg.attachments:
                att = await i.to_file()
                await channel.send(details, file=att)
    ...
    if msg.channel.id in [
            int(os.environ['discord_channel_beaneyboo_announcement'])
    ]:
        await asyncio.sleep(3)
        await msg.publish()
...
@bot.command(name='hello',
             aliases=['hi'],
             description="Greet user with Hello!")
...
@bot.command(name='guild',
             description="Return the current Server ID. For debugging.",
             hidden=True)
...
@bot.command(name='emote',
             description="Send any emote from servers the bot is in.")
...
@bot.command(
    name='weather',
    description="Send animated weather satelite images as mp4 video file.")
async def send_weather(ctx, action='sw'):
    if 'weather' not in sys.modules:
        await ctx.send(
            content=
            "Weather module was not imported. Please check console for problems."
        )
        return None
    await debug(cmsg="Requested weather command")
    await ctx.reply(
        content="Please wait 15 seconds. Generating __requested__ images.",
        mention_author=True)
    ...
    await ctx.send(
        content=
        "Please use weather command sparingly. Bot might get banned for using too much resource.",
        file=discord.File('out.mp4'))
    await debug(pmsg=4)
    weather.clean("target")
    await debug(pmsg=0)
...
async def bitch_react(ctx):
    await ctx.reply(content=await bot_startled(str(ctx.author.id)),
                    mention_author=False)


@bot.command(name='vca',
             description='Start Discord Together / Voice Chat Activities')
async def vca(ctx, app_name=None):
    try:
        vcid = ctx.author.voice.channel.id
    except AttributeError:
        await ctx.reply("Join a Voice Channel that I can see.",
                        delete_after=10)
        return
    if (app_name != 'list') and (app_name is not None):
        ...
        embed = discord.Embed(
            title="Discord Together Applications",
            description="Available applications for Discord Together")
        for i in list(set(valid_app.values())):
            embed.add_field(name=i,
                            value=[k for k, v in valid_app.items() if v == i],
                            inline=False)
        embed.set_footer(
            text="Example: `db vca youtube` to start a Watch Together session."
        )
...
async def roll_cmd(ctx, dice_notation: str):
    ...
    content = f"{result['prev']} + ({result['value'] - result['initial']}) = {result['value']}" if 'initial' in result.keys(
    ) else f"{result['prev']} = {result['value']}"
    await ctx.reply(content=content, mention_author=False)


@bot.command(name="debug",
             description="Send a test debug message",
             hidden=True)
async def command_debug(ctx, *, message: str = None):
    ...
    await on_command_error(
        ctx, error=message if message is not None else "Test error")
...
async def test(ctx):
    embed = discord.Embed(title="Embed test",
                          description=":p",
                          colour=discord.Colour(0xd6b4d8))
...
async def slash_vca(ctx, app_name: Option(str,
                                          "Enter the app name",
                                          choices=[
                                              'youtube', 'poker', 'chess',
                                              'betrayal', 'fishing',
                                              'letter-league', 'word-snack',
                                              'sketch-heads', 'spellcast',
                                              'awkword', 'checkers'
                                          ],
                                          default=None)):
    try:
        ...
        await ctx.respond("Join a Voice Channel that I can see.",
                          delete_after=10)
        return
...
async def slash_weather(ctx, action='sw'):
    ...
    if action == 'clean':
        weather.clean()
        await ctx.respond(content="Cache of weather images has been cleared.",
                          ephemeral=True)
    else:
        await debug(cmsg="Requested weather command")
        await ctx.respond(
...
@bot.slash_command(name="announcement",
                   description="Send an announcement to a channel.")
@default_permissions(moderate_members=True)
async def slash_announcement(ctx,
                             content: str,
                             test: bool,
                             tag: str = ['here', 'everyone'],
                             colour: int = 0xffb9d2,
                             channel_id: str = None):
    ...
    embed.set_footer(
        text=
        f"Published: {datetime.datetime.now(datetime.timezone.utc).astimezone(pytz.timezone('Asia/Manila')).strftime('%b %d, %Y :: %I.%M.%S %p')}"
    )
    ...
    if (channel_id is not None) and (test is False):
        ...
        if len(channel_id) == 1:
            ...
            if (cnl.permissions_for(cnl.guild.get_member(
                    ctx.author.id)).send_messages):
                await cnl.send(content=tag_switcher.get(tag, None),
                               embed=embed)
            ...
            respond = f"Announcement sent to {chnl_num} channels." if len(
                not_sent
            ) == 0 else f"Announcement sent to {chnl_num} channels.\nCould not send to these channels: `{not_sent}`"
            await ctx.respond(content=respond)
    else:
        await ctx.respond(content=tag_switcher.get(tag, None),
                          embed=embed,
                          ephemeral=False if test == False else True)
...
@bot.slash_command(name="analyze",
                   description="Check if link exists in Malicious Databases")
async def slash_analyze(ctx, domain: str):
    with requests.get(
            f"https://phish.sinking.yachts/v2/check/{domain}") as req:
        answer = f"{domain} is present in the Sinking Yachts anti-phishing list, and is therefore a *MALICIOUS* link." if req.json(
        ) is True else f"`{domain} was not found in the list. Be careful if you don't trust the link`"
...
async def slash_shutdown(ctx, reason=None, schedule=5, silent: bool = None):
    ...
    try:
        ...
        return None
    embed = discord.Embed(title=f"Scheduled Shutdown",
                          description=f"Shutdown in {shutdown_sched} seconds.",
                          colour=int(0xffb9d2))
    embed.add_field(
        name="Reason:",
        value=reason if reason is not None else "No reason stated.")
...
@bot.slash_command(name="emote",
                   description="Send an emote then an optional message.")
...
async def on_command_error(ctx, error):
    bad_cmd = (commands.MissingRequiredArgument, commands.BadArgument,
               commands.BadBoolArgument, commands.TooManyArguments)
    ...
    if isinstance(error, commands.CommandNotFound):
        ...
        embed = discord.Embed(title="Error in command",
        ...
        embed = discord.Embed(title="Unhandled custom pycord error",
                              description=f"Command: {ctx.command}")
        embed.add_field(name="Details:", value=f"{error}")
        embed.set_footer(
            text=
            f"Message: {ctx.message.content if ctx.message is not None else error}"
        )
        await ctx.send(embed=embed,
                       reference=ctx.message,
                       mention_author=False)
        await debug_cnl.send(content=f"Message URL: {ctx.message.jump_url}" if
                             ctx.message is not None else f"Message: {error}",
                             embed=embed)

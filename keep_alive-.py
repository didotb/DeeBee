...
def invite():
	'''if int(totp_data) == totp(os.environ['totp']):
		return f"https://discord.com/api/oauth2/authorize?client_id={os.environ['discord_bot_id']}&permissions={request.args.get("perms")}&scope=applications.commands%20bot"'''
	data = request.json
	return data
